...
impl::Socket *HttpSocket<isServer>::onData(impl::Socket *s, char *data, size_t length) {
    ...
    if (FORCE_SLOW_PATH || httpSocket->httpBuffer.length()) {
        ...
        httpSocket->httpBuffer.reserve(httpSocket->httpBuffer.length() + length + WebSocketProtocol<CLIENT, WebSocket<CLIENT>>::CONSUME_POST_PADDING);
    ...
    if (isServer) {
        *perMessageDeflate = false;
        ...
        if (extensionsLength) {
            ...
            ExtensionsNegotiator<SERVER> extensionsNegotiator(group->extensionOptions);
